...
public class SynchronizedQueue<T> {
	...
	public T dequeue() {
		synchronized(this) {
	        while (in - out <= 0) { 	//queue is empty
	            if (beforeFirst || producers != 0) {
	                try {
	                    this.wait();
	                } catch (Exception e) {
	        ...
	        // queue is not empty
	        T res = buffer[out++ % buffer.length];
	        this.notifyAll();
	        return res;	
		}
	}
	...
	public void enqueue(T item) {
		synchronized(this) {
			while (in - out >= buffer.length) {		//queue is full
	            try {
	                this.wait();
	            } catch (Exception e) {
	                System.out.println(e);
	...
	/**
	 * Returns the total amount of elements enqueued so far
	 */
	public int getCount() {
		return in;
		}
	...
	public void printQueue() {
		for (int i = 0; i < this.getSize(); i++) {
			System.out.println(buffer[(out + i) % this.getSize()]);
		}
	}
}
